package ru.ccfit.inechakhin.tests;
...
public class UrlParserCorrectRFC {
    ...
    public void testNetURL(String url, JsonObject partUrl) throws MalformedURLException {
        ...
        if ((parsingUrl.getAuthority() == null && partUrl.get("authority") != null) || (parsingUrl.getAuthority() != null && partUrl.get("authority") != null && !partUrl.get("authority").equals(parsingUrl.getAuthority()))) {
            System.out.println("Problem in authority:\n" + url + "\n" + partUrl.get("authority") + "\n" + parsingUrl.getAuthority());
        }
    ...
    public void run() throws IOException, JsonException, URISyntaxException {
        Reader reader = Files.newBufferedReader(Paths.get("src/main/resources/fuzz.json"));
